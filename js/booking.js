...
(function () {
  ...
  // ─── Настройки ─────────────────────────────────────────────────────────────
  // ⚠️ Адрес намеренно НЕ `zapis.tiemulaoshi.ru`, хотя такой поддомен
  // настроен и работает. Проверено 03.08.2026: с мобильных сетей соединение
  // с ним зависает (белая страница, запрос не завершается), при этом тот же
  // сервер по адресу *.up.railway.app открывается нормально. Разница только
  // в имени, то есть фильтрация идёт по имени сайта, а не по адресу сервера.
  // Виджет из-за этого уходил в запасной вариант на всех телефонах.
  // Вернуть можно только после того, как причина найдена и устранена.
